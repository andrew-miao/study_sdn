    ...
    def _packet_in_handler(self, ev):
       ...
       # self.vlan_to_port.setdefault(dpid, {})
        ...
        if in_port in access_ports[dpid - 1]:
            ...
            print(" the type of pkt is", type(pkt))
            if pkt != None:
                eth = pkt.get_protocols(ethernet.ethernet)[0]
            else:
                return
        ...
        for p in pkt:
            ...
            if type(p) != type('string'):
                print(" check the protocol name", p.protocol_name)
                proto_names.append(p.protocol_name)
            else:
                continue
        src_vlan = 'NULL'
        if len(proto_names) != 0:
            for pkt_name in proto_names:
                print(" the protocols is", pkt_name)
                if pkt_name == 'vlan':
                    src_vlan = p.vid
        ...
        if in_port in access_ports[dpid - 1]:
            ...
            actions.append(parser.OFPActionOutput(out_port))
            self._send_normal_packet_(datapath, msg, src, dst, in_port, 1, actions, out_port_type)
